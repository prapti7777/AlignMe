...
function toggleAuthMode() {
    ...
    const mainBtn = document.getElementById('auth-main-btn'); // Matches HTML ID
    const toggleText = document.getElementById('auth-toggle-text');
    const toggleLink = document.getElementById('auth-toggle-link');

    if (isLoginMode) {
        tagline.innerText = "Sign in to start your career journey";
        mainBtn.innerText = "Login";
        toggleText.innerText = "Don't have an account?";
        toggleLink.innerText = "Register";
    } else {
        tagline.innerText = "Create your account to save your roadmaps";
...
// Renamed to handleAuthAction to match your HTML's onclick="handleAuthAction()"
async function handleAuthAction() {
    // Corrected IDs to match your HTML: "auth-username" and "auth-password"
    ...
    try {
        // Pointing explicitly to your Flask port 5000
        ...
        if (response.ok) {
            ...
            setStatusMessage(result.error || "Something went wrong", true);
        }
    } catch (error) {
...
function setStatusMessage(message, isError = false) {
    const msgElement = document.getElementById('auth-status-msg');
    
    msgElement.innerText = message;
    msgElement.classList.remove('status-hidden');
    
    // Apply red color if it's an error, otherwise teal
    msgElement.style.color = isError ? "#d9534f" : "var(--primary-teal)";

    // Auto-hide after 3.5 seconds
    setTimeout(() => {
        msgElement.classList.add('status-hidden');
    }, 3500);
}
